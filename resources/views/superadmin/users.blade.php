@php
    $roleLabel = ($roleFilter === 'mitra') ? 'Mitra' : (($roleFilter === 'kustomer' || $roleFilter === 'customer') ? 'Customer' : 'User');
    $title = 'Manajemen ' . $roleLabel;
    $breadcrumb = 'Super Admin / ' . $title;
@endphp
    ...
    <div class="bg-white rounded-2xl border border-gray-200/80 p-3 sm:px-5 sm:py-3.5 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
        <div class="flex items-center gap-3">
            ...
            <span class="text-base sm:text-lg font-bold text-gray-900">
                Jumlah {{ strtolower($roleLabel) }} saat ini: <span class="text-blue-600 font-extrabold ml-1">{{ $users->total() }}</span>
            </span>
        </div>
                        ...
                        @forelse($users as $user)
                            <tr class="hover:bg-gray-50/80 transition-colors">
                                ...
                                <td class="px-2.5 py-3 whitespace-nowrap">
                                    @php
                                        $roleColors = [
                                            'mitra' => 'bg-green-50 text-green-700 border border-green-200',
                                            'kustomer' => 'bg-gray-100 text-gray-700 border border-gray-200',
                                        ];
                                        $roleLabels = [
                                            'mitra' => 'Mitra',
                                            'kustomer' => 'Customer',
                                    ...
                                    @endphp
                                    <span class="px-2 py-0.5 inline-flex text-[11px] font-semibold rounded-full {{ $roleColors[$user->role] ?? 'bg-gray-100 text-gray-700 border border-gray-200' }}">
                                ...
                                <td class="px-2.5 py-3 text-center whitespace-nowrap">
                                    @php
                                        if ($user->isMitra()) {
                                            $avgRating = $user->mitra_average_rating;
                                            $ratingCount = $user->mitra_rating_count;
                                        } else {
                                            $avgRating = $user->customer_average_rating;
                            ...
                            <tr>
                                <td colspan="{{ $roleFilter ? 9 : 10 }}" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-gray-400 text-xs mt-0.5">Coba ubah filter atau tambah user baru</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
